import"./modulepreload-polyfill-B5Qt9EMX.js";import"./auth-B0p6mXTA.js";document.addEventListener("DOMContentLoaded",()=>{const d=document.getElementById("studentSearchInput"),i=document.getElementById("searchResults"),u=document.getElementById("profile-search-section"),c=document.getElementById("profile-details-section");let g,l=null;const m=new URLSearchParams(window.location.search).get("tr");m&&p(m),d.addEventListener("input",e=>{const t=e.target.value.trim();if(clearTimeout(g),t.length<2){i.style.display="none";return}g=setTimeout(()=>f(t),300)});async function f(e){try{const a=await(await fetch(`/api/staff/student-search?q=${encodeURIComponent(e)}`)).json();y(a.success?a.data:[])}catch(t){console.error("Search fetch error:",t)}}function y(e){!Array.isArray(e)||e.length===0?i.innerHTML='<div class="search-result-item">No students found.</div>':i.innerHTML=e.map(t=>`
            ...
            <div class="progress-card">
                ...
                <div class="progress-info">
                    ...
                    <p>${o.completed?"All tiers done!":`${o.current} / ${o.target} hrs`}</p>
                </div>
